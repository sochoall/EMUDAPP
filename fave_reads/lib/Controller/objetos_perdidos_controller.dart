...
class ObjetosPerdidosController extends ResourceController {
  ...
  Future<Response> obtenerLista() async {
    //print("ENTRA EN EL OBJETOS PERDIDOS CONTROLLER CONSULTA TODO");
    final servicio = ObjetosPerdidos();
    return Response.ok(await servicio.obtenerDatos());
  }

  @Operation.get('fechas')
  Future<Response> obtenerListaFecha(@Bind.path('fechas') String fechas) async 
  {
    print("ENTRA EN EL OBJETOS PERDIDOS CONTROLLER");
    final servicio = ObjetosPerdidos();
    return Response.ok(await servicio.obtenerDatoFechas(fechas));
  }
 ...
 /* @Operation.get('id')
  ...
  @Operation.post()
  Future<Response> crearObjetosPerdidos(
      @Bind.body() ObjetosPerdidos body) async {
    final servicio = ObjetosPerdidos();
  ...
  @Operation.put('id')
  Future<Response> modificarObjetosPerdidos(
      @Bind.path('id') int id, @Bind.body() ObjetosPerdidos body) async {
    final servicio = ObjetosPerdidos();
