...
class TipoInstitucion extends Serializable
{
  ...
  Future<List> obtenerDatos() async {
    ...
    const String sql = "select * from public.te_tipo_institucion where tin_estado=0";
    final List datos=[];
  ...
  Future<TipoInstitucion> obtenerDatoId(int id) async {
    final conexion = Conexion();
    final String sql = "select * from public.te_tipo_institucion where tin_id=$id";
  ...
  Future<void> ingresar(TipoInstitucion dato) async{
    final conexion = Conexion();
    final String sql = "INSERT INTO public.te_tipo_institucion(tin_id,tin_nombre, tin_estado) "
   ...
   Future<void> modificar(int id,TipoInstitucion dato) async{
    final conexion = Conexion();
    final String sql = "UPDATE public.te_tipo_institucion SET tin_nombre='${dato.nombre}' WHERE tin_id=$id";
    await conexion.operaciones(sql);
  }

   Future<void> eliminar(int id) async{
    final conexion = Conexion();
    final String sql = "UPDATE public.te_tipo_institucion SET tin_estado=1 WHERE tin_id=$id";
    await conexion.operaciones(sql);
  }
